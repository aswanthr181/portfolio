...
import { data, miniProject } from './ProjectData'
import styled from 'styled-components'
import { RxExternalLink } from 'react-icons/rx'
import { AiOutlineGithub } from 'react-icons/ai'
import { Link } from 'react-router-dom'
function ProjectCard({ width }) {

  const [reversedData, setReversedData] = useState([])
  // const reversedData = data
  useEffect(() => {
    { width == '380px' ? setReversedData(data) : setReversedData(miniProject) }
  }, [])
  ...
  return (
    <>
      {reversedData.map((data) => {
        return (
          <div data-aos="zoom-in" key={data.id} className="flex flex-col justify-center items-center gap-10">
            <POPUP className="img-content relative">
              <div className={`h-[280px] w-[${width}] hover:scale-125 transition duration-500 cursor-pointer shadow-xl rounded-md overflow-hidden sm:h-[260px] sm:w-[92%] sm:bg-cover mx-auto`} >
                {/* <div className="h-[280px] w-[250px] hover:scale-125 transition duration-500 cursor-pointer shadow-xl rounded-md overflow-hidden sm:h-[260px] sm:w-[92%] sm:bg-cover mx-auto"> */}
